...
## Graphify for Codebase Documentation

When the user requests documentation about how components, screens, functions, or any part of the codebase works, **ALWAYS use Graphify** instead of manually reading files or searching through code. Graphify builds a knowledge graph of the project and provides accurate, relationship-aware answers.

### When to use Graphify
Use Graphify for ANY documentation request involving:
- How a feature or component works
- How screens/pages are structured and their flow
...
| Command | Use Case |
|---------|----------|
| `graphify query "<question>"` | Ask how something works in plain language |
| `graphify path "<A>" "<B>"` | Find how two components/classes are connected |
| `graphify explain "<concept>"` | Get detailed explanation of a specific concept |
| `graphify update .` | Rebuild graph after code changes |

### Workflow for Technical Documentation Requests
1. First, ensure the graph is up to date: `graphify update .`
2. Query the graph: `graphify query "<user's question>"`
3. If needed, trace specific relationships: `graphify path "ComponentA" "ComponentB"`
4. For deep dives on specific concepts: `graphify explain "ConceptName"`
5. Synthesize the graph output into clear documentation

### Graph Location
The knowledge graph is stored in `graphify-out/`:
- `graph.json` — full queryable graph
- `GRAPH_REPORT.md` — architecture overview
- `graph.html` — interactive visualization (when under 5000 nodes)

### Important Notes
- The graph is built automatically via AST parsing (no LLM cost for code)
- Always run `graphify update .` before querying if code has changed
- For broad architecture review, read `graphify-out/GRAPH_REPORT.md`
- For specific questions, prefer `graphify query` over manual file reading
...
- Architecture documentation for a specific part of the system
