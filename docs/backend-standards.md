---
description: Backend development standards, best practices, and conventions for the Chapur Pay Spring Boot Java WAR application, including Oracle access, stored procedures, Spring MVC, DWR services, and testing practices
globs: ["src/main/java/**/*.java", "src/main/resources/**/*.{properties,xml}", "src/main/webapp/**/*.{jsp,js,css}", "src/test/**/*.java", "pom.xml"]
alwaysApply: true
---
...
- [Overview](#overview)
- [Chapur Pay Runtime and Connection Rules](#chapur-pay-runtime-and-connection-rules)
  - [Core Database Connection](#core-database-connection)
...
This document outlines the best practices, conventions, and standards used by the Chapur Pay backend. The application is a layered Spring Boot WAR with Java services, repositories, Spring MVC controllers, DWR endpoints, and JSP views.

## Chapur Pay Runtime and Connection Rules

### Core Database Connection

The Chapur Pay core connects to the Oracle `VENTAS` database through Spring Boot's managed datasource. Database access belongs in repositories and services, never in JSP, JavaScript, controllers, or DWR clients.

- Use `spring.datasource.*` properties from the active Spring profile.
- Use the Oracle driver (`oracle.jdbc.OracleDriver`) and `oracle.jdbc.pool.OracleDataSource` configured in `application.properties`.
- Keep the Oracle dialect and schema conventions used by the application (`org.hibernate.dialect.Oracle10gDialect` and the `VENTAS` schema).
- Use the injected `JdbcTemplate`, JPA `EntityManager`, or existing repository abstractions. Do not create `DriverManager`, `DataSource`, or `JdbcTemplate` instances inside a DAO method.
- Preserve the existing stored-procedure contract. Procedures are invoked in the `VENTAS` schema and must receive explicitly mapped input, output, and cursor parameters.
- Do not use `ddl-auto=create`, `update`, or `create-drop`; the application is configured with `ddl-auto=none` because schema changes are managed by the database team.

Example of the expected profile-based configuration. Values are placeholders and must be supplied by deployment configuration or a secret manager:
...
# application-dev.properties, application-qa.properties, or application-prod.properties
spring.datasource.url=${CHAPUR_DB_URL}
spring.datasource.username=${CHAPUR_DB_USERNAME}
spring.datasource.password=${CHAPUR_DB_PASSWORD}
```

Never commit database usernames, passwords, tokens, or complete production JDBC URLs to source control. If an existing file contains a credential, rotate it and move the value to deployment secrets.

### Environment Profiles

The active profile determines both the database endpoint and the behavior of external Chapur services:

| Profile | `parametro.ambiente.prueba` | Use | Rule |
| --- | --- | --- | --- |
| `dev` | `true` | Local development | Use only the development Oracle endpoint and development services. |
| `qa` | `true` | QA validation | Use only the QA Oracle endpoint and QA services. |
| `prod` | `false` | Production | Use production endpoints and never run test data or test-only flows. |

Start the application with an explicit profile instead of relying on an IDE default:

```bash
mvn spring-boot:run -Dspring-boot.run.profiles=dev
# or
java -jar chapurPay.war --spring.profiles.active=qa
```

When adding a new environment, create a new `application-<profile>.properties` file and define the datasource, service URLs, context path, and `parametro.ambiente.prueba` value together. Do not select a profile by changing Java conditionals or by editing repository code.

### Testing Database Access

Tests must not connect to the shared Chapur Oracle database by default. Apply the following conditions:

- Unit tests mock `JdbcTemplate`, `EntityManager`, repositories, and external service clients; they verify SQL/procedure names, parameters, mapping, errors, and transaction behavior without network access.
- Integration tests require an explicit test profile or an explicit opt-in environment variable. They must never inherit `prod`, and they must use an isolated schema or disposable test database approved for testing.
- Do not place database passwords in `src/test`, test fixtures, logs, or test reports.
- The project currently has no `src/test` suite and `maven-surefire-plugin` is configured with `<skipTests>true</skipTests>`. Any new tests must be added under `src/test/java`, and the skip configuration must be deliberately overridden in CI before tests are considered executed.

Example of an explicit opt-in test command:

```bash
mvn test -DskipTests=false -Dspring.profiles.active=test
```

If a `test` profile is introduced, it must fail fast when its datasource variables are missing and must not silently fall back to `dev`, `qa`, or `prod`.

## Technology Stack

### Core Technologies
- **Java 8**: Application language and runtime compatibility target
- **Spring Boot 2.6.3**: Application framework and configuration
- **Maven**: Dependency management, compilation, packaging, and execution
- **WAR packaging**: Deployable artifact named `chapurPay.war`
- **Spring MVC**: HTTP request routing and controller layer
- **Spring Boot Actuator**: Health and information endpoints

### Database & ORM
- **Oracle Database**: `VENTAS` schema used by Chapur Pay
- **Oracle JDBC**: `ojdbc8` version `23.2.0.0` and `oracle.jdbc.OracleDriver`
- **Spring Data JPA / Hibernate**: Persistence infrastructure using `Oracle10gDialect`
- **Spring JDBC**: `JdbcTemplate` for repository access and stored procedures
- **JPA `EntityManager`**: Direct stored-procedure calls with input, output, and REF_CURSOR parameters
- **Schema management**: Database-owned; Hibernate `ddl-auto` is `none`

Database access is profile-based and must remain in the Java core. JSP and browser JavaScript never connect directly to Oracle.

### Testing Framework
- **Spring Boot Starter Test**: Spring test utilities and assertions
- **JUnit 4.11**: Existing unit test API
- **Mockito 1.9.5**: Existing mocking library
- **Maven Surefire**: Test execution; currently configured with `<skipTests>true</skipTests>`
- **Test Location**: `src/test/java`
- **Database test policy**: Unit tests mock JDBC/JPA access; integration tests require explicit opt-in and an isolated database

### Development Tools
- **Lombok**: Getters, setters, and bean boilerplate
- **JSTL**: JSP tag library support
- **DWR 3.0.2**: Java-to-browser remote services
- **Apache HttpClient**: HTTP integration support
- **Redis Session**: Spring Session Data Redis dependencies for shared session support
- **iText**: PDF generation
- **Apache POI**: Excel generation
- **GlassFish-compatible servlet deployment**: `glassfish-web.xml`; embedded Tomcat is provided for development/container integration
...
**Infrastructure Layer** (`src/main/java/com/chapur/sdc/repository/`)
- Spring JDBC and JPA handle Oracle database operations
- DAOs invoke `VENTAS` stored procedures through injected infrastructure beans

### Project Structure

```
chapur_pay_plataforma/
├── src/main/java/com/chapur/sdc/
│   ├── configuration/       # Spring and application configuration
│   ├── controller/          # Spring MVC controllers
│   ├── service/             # Business and DWR services
│   ├── repository/          # DAOs and Oracle access
│   ├── model/               # Beans, DTOs, and stored-procedure models
│   └── commons/             # Shared utilities and base classes
├── src/main/resources/
│   ├── application*.properties  # Profile-specific configuration
│   └── dwr.xml               # DWR integration
├── src/main/webapp/WEB-INF/jsp/ # Server-rendered JSP views
├── src/main/webapp/resources/   # JavaScript, CSS, images, and libraries
├── src/test/java/             # Java tests when enabled
├── pom.xml                    # Maven build and dependencies
└── glassfish-web.xml          # WAR/container configuration
```
...
mvn spring-boot:run                 # Run with the configured/default profile
mvn spring-boot:run -Dspring-boot.run.profiles=dev
mvn package                         # Build the chapurPay.war artifact
mvn test -DskipTests=false          # Run tests explicitly
mvn clean package                   # Clean build and package
```

### Code Quality

- **Maven Compilation**: Ensure Java sources compile without errors
- **Configuration Validation**: Verify the selected profile and external secrets before startup
- **Test Execution**: Do not treat tests as executed while Surefire skip is enabled
- **All Tests Passing**: Ensure all enabled tests pass before deployment
- **Code Review**: Review code for adherence to standards

This document serves as the foundation for maintaining code quality and consistency across the Chapur Pay Spring Boot WAR. All team members should follow these practices to ensure a maintainable, scalable, and testable codebase.
